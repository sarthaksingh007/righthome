  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-4 cursor-pointer">
        {isLoading ? (
          <span>Loading...</span> // Show loading text while fetching user data
        ) : error ? (
          <span>Error loading user data</span> // Handle any errors
        ) : user ? (
          <>
            <img
              src={user.picture || "/assets/profile.jpeg"} // Use user picture if available, otherwise fallback
              alt="User Profile"
              className="h-10 w-10 rounded-full border-gray-400 border-2 mr-2 fixed right-4"
            />
            <span className="text-sm">{user.name || user.email}</span> {/* Display user's name or email */}
          </>
        ) : (
          <span>Not logged in</span> // Show message if user is not logged in
        )}
      </div>
    </div>
  );
